...
def processRequestParameter(field, value, processing_list=[], decryption_list=[]) -> dict:
    try:
    ...
    try:
        ...
        logger.error("The payload could not be processed automatically. It may be getting recognized repeatedly by a data processor. You should force the coding parameters.")
    ...
    try:
        ...
        if config['auto_process'].get():
            ...
            logger.error(f'There was an issue when attempting to decode the processedData. Is the data encoded or encrypted properly? Decode Order: {processing_list} - Decrypt Order: {decryption_list}')
